...
public class FibonacciHeap {
     ...
     */
     ...
     public void decreaseKey(HeapNode x, int diff) {//copied
        ...
        if (y != null && x.key < y.key) {
            cut(x);
            cascadingCut(y);  // Non-recursive: at most marks the parent or does nothing
        }
        if (x.key < this.min.key) {
            this.min = x;
        }
    ...
    public void decreaseKey(HeapNode x, int diff) { //Yoad    
        ...
        */
        private void cut(HeapNode x) {//copied
            this.totalCuts++;
            this.numTrees++;
    ...
    private void cut(HeapNode x) { //Yoad
        ...
        */
        ...
        private void cascadingCut(HeapNode y) {//copied
            if (y.parent == null) {
                return;  // y is a root => nothing more to do
            }
            if (!y.mark) {
                y.mark = true;  // Just set the mark
            } 
            // If y is already marked, do nothing here 
            // (i.e., no recursive cut(y), no further cascading).
        }
    /* 
